...
- **(2026-09-18, v2.6)** Un proyecto concreto corrió un examen end-to-end del sistema completo (5 conversaciones reales contra el motor central real, no simuladas) y encontró un defecto real en la implementación de referencia de la sección 6.7 (`DeterministicLeadExtractor`), ya corregido en ese proyecto — se generaliza aquí para que ningún otro proyecto que haya sembrado este molde lo repita:
  - **Defecto:** un patrón de captura "frase disparadora + hasta N palabras" (ej. `soy|me llamo|my name is` seguido de 1-3 palabras) no tiene ningún concepto de límite de oración. Cuando un huésped dice el nombre y el siguiente dato en el mismo aliento ("Mi nombre es X y mi correo es...", "I'm X and my email is..."), la palabra conectora (y/e/and) — y, si cae antes del límite de N palabras, lo que sea que la siga — se captura como si fuera parte del nombre.
  - **Corrección generalizable:** validar cada palabra capturada contra una lista de conectores conocidos (no solo la primera palabra, que es lo único que la Guarda de Stopwords de la sección 6.7 ya cubría) y truncar la captura en la primera ocurrencia, sin importar en qué posición del grupo caiga — nunca solo revisar/recortar la última palabra, un huésped puede meter la conectora en cualquier posición dentro del límite de N palabras.
  - **Segundo hallazgo, no corregido (documentado como brecha conocida, no como regla nueva):** el mismo extractor, por diseño, nunca captura un nombre dado como texto plano sin la frase disparadora — un huésped que responde solo "Ana García" a "¿con quién tengo el gusto?" no queda capturado, aunque el modelo central sí trate el dato como recibido en su propia respuesta conversacional. Ampliar la captura a "texto plano inmediatamente después de que el asistente lo pidió" requeriría que el extractor determinístico reciba también el texto del asistente (hoy solo recibe el del huésped) y conlleva riesgo real de falsos positivos — se documenta como límite de diseño conocido, no como defecto a corregir a la ligera, misma cautela Anti-Alucinación que ya gobierna el resto de esta sección.
  - Ningún nombre de marca, ruta comercial, ni dato de un tenant real se incorporó en este proceso — el hallazgo real, su reproducción exacta y su verificación en vivo (incluyendo contra producción) viven en el Codex de ese proyecto concreto, no aquí.
- **(2026-09-19, v2.7)** El mismo proyecto concreto de v2.6 completó una segunda ronda de examen (idiomas no cubiertos por la sección 2.3, Escalación White-Glove §3.2/§4, y el límite de vistas del Enlace Efímero §8.1) y encontró dos defectos adicionales generalizables, ambos ya corregidos ahí:
  - **Defecto (Enlace Efímero, `EphemeralAccessTokenManager`, sección 6.5):** una implementación concreta de referencia con la forma `UPDATE ... SET view_count = view_count + 1, status = IF(view_count + 1 >= max_views, 'expired', status) WHERE ...` expira el recurso una vista antes de lo prometido. Causa raíz agnóstica del motor SQL, no del dominio de este molde: en una sentencia `UPDATE` con múltiples columnas, la mayoría de motores relacionales evalúan las asignaciones de izquierda a derecha dentro de la misma sentencia — para cuando se evalúa la expresión de una columna posterior, una columna reasignada antes en el mismo `SET` ya vale su valor nuevo, no el previo a la actualización. La condición del `IF` sumaba el `+1` una segunda vez sin darse cuenta. **Corrección generalizable:** si una expresión de una columna posterior en el mismo `SET` necesita el valor *nuevo* de una columna reasignada antes, esa expresión ya lo tiene sin sumar de nuevo el delta — sumarlo otra vez es el bug. Cualquier molde de referencia con este patrón "contador + bandera de estado derivada del contador, en el mismo UPDATE" debe auditarse contra este mismo error.
  - **Segundo hallazgo (Cerrojo Comercial #2 / White-Glove Escalation, sección 3.2):** confirmado 2 de 2 en pruebas independientes contra el motor central real — el modelo genera de forma confiable el texto conversacional correcto ("notificaré a Lester" / equivalente) al detectar una señal de escalación, pero no emite de forma confiable el marcador `[[...ESCALATE]]` que realmente dispara el efecto server-side (bandera VIP + alerta interna). Mismo patrón de desconfianza ya documentado en la sección 1.5.1 y generalizado en la sección 4.4/6.7 para la extracción de datos de lead — aquí se confirma que **también aplica al mecanismo de escalación**, no solo a la extracción de campos. **Recomendación generalizable, no implementada en ningún proyecto todavía:** el mismo patrón "post-procesamiento determinístico, nunca delegado al modelo" que ya gobierna 4.4/6.7 debería extenderse a la detección de las señales de escalación (perfil ejecutivo, tamaño de evento, mención de un activo insignia específico) como una capa de respaldo server-side, no como reemplazo del marcador — el modelo sigue intentando emitirlo, pero un escaneo determinístico del texto del huésped no depende de que lo logre.
  - Ningún nombre de marca, ruta comercial, ni dato de un tenant real se incorporó en este proceso — el detalle real vive en el Codex de ese proyecto concreto, no aquí.
- **(2026-09-19, v2.8)** Un proyecto concreto amplió su simetría de idioma de EN/ES a 5 idiomas (agregando los 3 más relevantes para su mercado turístico) y, al probarlo, encontró dos huecos de captura en `DeterministicLeadExtractor` (sección 6.7) directamente causados por esa ampliación — corregidos ahí, generalizados aquí:
  - **Regla nueva:** cualquier proyecto que amplíe el rango de idiomas soportados en la Capa A (sección 2.3) debe auditar en la MISMA iteración las listas de frases disparadoras del extractor determinístico (nombre, y cualquier otro campo capturado por frase disparadora) — ampliar un lado sin el otro deja huésped en el idioma nuevo con una conversación perfecta pero datos de contacto silenciosamente vacíos, un hueco peor que no soportar el idioma en absoluto (el guest cree que ya quedó todo capturado).
  - **Defecto separado, mismo hito:** un patrón de fecha en inglés con `preg_match()` (no `preg_match_all()`) sobre "palabra + número" fallaba cuando el texto del huésped contenía una coincidencia previa no-válida antes de la fecha real (ej. "...for 3 people on December 8th" — coincide primero con "for 3", nunca llega a examinar "December 8"). `preg_match()` solo entrega la primera coincidencia de todo el texto — cualquier extractor de este patrón (palabra candidata + validación posterior contra una lista fija) debe iterar TODAS las coincidencias con `preg_match_all()`, nunca conformarse con la primera y rendirse si no es válida.
  - Ningún nombre de marca, ruta comercial, ni dato de un tenant real se incorporó en este proceso — el detalle real vive en el Codex de ese proyecto concreto, no aquí.
- **(2026-09-19, v2.9)** El mismo proyecto concreto probó a fondo su canal secundario (WhatsApp, sección 4.2/6.3) contra el canal principal (widget web) que hasta ahora era el único validado de punta a punta, y encontró un defecto de paridad entre canales grave, ya corregido ahí:
  - **Regla nueva, la más importante de esta entrada:** cuando el mismo Gateway Central (sección 1.6) se normaliza a OCMC desde más de un canal de entrada (widget web, WhatsApp, y cualquier canal futuro — Telegram, redes sociales), **cada punto de entrada debe invocar exactamente la misma secuencia de post-procesamiento** (sentinels §3.4/§6.6, extracción determinística §4.4/§6.7) — no solo compartir las tablas de persistencia. Un proyecto concreto construyó el canal de WhatsApp como una implementación paralela del mismo contrato en vez de un caso más de un único punto de post-procesamiento compartido, y ese paralelismo dejó silenciosamente sin ejecutar el paso de extracción de lead solo en ese canal — la conversación funcionaba perfecto (el modelo capturaba y usaba los datos con naturalidad), pero nada quedaba persistido de forma estructurada. **Recomendación de diseño para nuevas instancias de este molde:** factorizar el post-procesamiento (sentinels + extracción + resumen) en una única función/paso que TODOS los adaptadores de canal llamen, en vez de que cada adaptador de canal la invoque por su cuenta — así una omisión como esta se vuelve estructuralmente imposible en vez de solo "hay que acordarse de llamarla en cada archivo nuevo".
  - **Verificado, sin defectos:** carga simultánea de varias sesiones distintas contra el mismo Gateway Central no mostró ninguna condición de carrera ni contaminación de datos entre sesiones — el diseño de una fila por sesión con identidad propia (sección 6.1) sostiene bien la concurrencia real, al menos a la escala probada (5 sesiones simultáneas).
  - Ningún nombre de marca, ruta comercial, ni dato de un tenant real se incorporó en este proceso — el detalle real vive en el Codex de ese proyecto concreto, no aquí.
